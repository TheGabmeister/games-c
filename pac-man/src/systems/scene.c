...
static inline void _pause(ecs_world_t *world)
{
  if (_paused == 0)
  {
    _paused = entity_manager_spawn_label(world, 0, FONT_CLOVER, "PAUSED", ALIGN_CENTRE, VALIGN_MIDDLE, 100, (vector2){RASTER_WIDTH * 0.5, RASTER_HEIGHT * 0.5}, (color){ 80, 80, 80, 255 });
    ShowCursor();
    if (_music != 0)
    {
